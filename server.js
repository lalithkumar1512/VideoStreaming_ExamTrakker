...
http.listen(port, function () {
	...
	mongoClient.connect("mongodb+srv://lalithkumar1512:chinnapinni@userdb.ni6slbq.mongodb.net/?retryWrites=true&w=majority", { useUnifiedTopology: true }, function (error, client) {
				...
				if (user == null) {
					...
					bcrypt.compare(password, user.password, function (error2, res) {
						if (res === true) {
							...
							localStorage.setItem("ID", user._id.valueOf());
							console.log(localStorage.getItem("ID"));
							result.redirect("/");
						} else {
							result.render("login", {
			...
			if (request.session.user_id) {
				...
				formData.parse(request, function (error1, fields, files) {
					...
					fileSystem.rename(oldPath, newPath, function (error2) {
						console.log("video upload error = ", error2)
						getUser(request.session.user_id, function (user) {
		...
		app.get("/bufferVideo/:id", function (request, result) {
				...
				if (video == null) {
					...
					database.collection("users").updateOne({
						...
						$inc: {
							"videos.$.views": 1,
							"videocount.$.count": 1,
							"watchtime": video.hours * 3600 + video.minutes * 60 + video.seconds
						}
					});
					...
					if (video == null) {
						...
						database.collection("videos").findOne({
							...
							fileSystem.unlink(videoData.filePath, function (errorUnlink) {
								...
								fileSystem.unlink(videoData.thumbnail, function (errorUnlink) {
									if (errorUnlink) {
										console.log(errorUnlink)
									}
